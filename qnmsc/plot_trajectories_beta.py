...
def load_data(npoles, scale_osc, scale_damping, domain):
    ...
    param = results['param']
    radius, beta = zip(*param)  # <-- Extract from param
    radius = np.array(radius)
    beta = np.array(beta)
    material_poles = to_eV(calc_material_poles(scale_osc, scale_damping))[:npoles]
    return poles, residues, radius, beta, material_poles
...
def plot_beta(npoles, scale_osc, scale_damping, domain, 
              unit="eV", inv=True, horizontal=False, use_alpha=True):
    poles, residues, radius, beta, material_poles = load_data(
        npoles, scale_osc, scale_damping, domain
    )

    for b, p, r in zip(beta, poles, residues):
        if p.size:
            p = unit_conversion[unit](p)
            y = 1/b if inv else b
            y = [y] * len(p)
    ...
    xlabel = f"$\\Re\\{{{qty_str[unit]}\\}}$ [{unit_str[unit]}]"
    ylabel = f"$1/\\beta$" if inv else f"$\\beta$"
...
if __name__ == "__main__":
    plt.figure()
    plot_beta(3, 1, 1, domain, inv=True, horizontal = True)
    plt.ylim(1, 2.5)
    plt.xlim(1.1, 2.0)
    plt.savefig("out/Elli_3pole_test.pdf", dpi=1200)
    plt.show()
