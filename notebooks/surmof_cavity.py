...
import archive.mie_smat as mie_smat
...
def ag_surmof_cavity_det_smat(wfreq, thickness, npoles:int = 3, scale_osc:float=1):
    ...
    # Here, we can decide what quantity to return.
    S = np.moveaxis(np.array([[ref, tr],[tr, ref2]]), -1, 0)
...
def ag_surmof_core_shell(wfreq, d_core, npoles:int = 3, l=1):
    ...
    coeffs = [mie_smat.mie(l, k * np.array([d_core/2, d_core/2+mirror]), [eps_c, eps_a, eps_air], [1,1,1], [0,0,0])[0,0] 
